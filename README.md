...
This project demonstrates an example flow for setting up subscription based payments for web3 apps using web3sdks [wallet authentication](https://docs.web3sdks.com/advanced-features/wallet-authentication) and [Stripe](https://stripe.com). 
...
Since we are using the `web3sdks authentication` flow, we'll also need to specify a domain in our environment variables for both our client and server sides to use. This domain should be the url of your client side application and is used to prevent phishing attacks. We can add it to the `NEXT_PUBLIC_AUTH_DOMAIN` variable in our `.env.local` file.
...
Now that we've setup our web3sdks app with authentication, we need to setup Stripe to handle payments.
...
- [web3sdks Wallet Authentication](https://docs.web3sdks.com/auth) - learn about our wallet authentication flow
